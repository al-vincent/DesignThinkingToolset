...
def get_regions(img_url):
    if img_url is not None:        
        ...
        logger.error(f"utilities.get_regions: No image URL in session data")
        return None
...
def generate_container_name():
    ...
    now = datetime.now()
    return f"{now.strftime('%Y%m%d%H%M%S')}-{uuid.uuid4()}"
...
def click_analyse_text_button(request):
    ...
    region_text = get_text(image_url, regions)
